...
class Config:
    def __init__(self, config_file="config.ini"):
        ...
        self.langfuse_secret_key: str = self.config["langfuse"].get(
            "LANGFUSE_SECRET_KEY"
        )
        self.langfuse_public_key: str = self.config["langfuse"].get(
            "LANGFUSE_PUBLIC_KEY"
        )
        ...
        self.home_assistant_base_url: str = self.config["homeassistant"].get(
            "HOME_ASSISTANT_BASE_URL"
        )
        self.home_assistant_token: str = self.config["homeassistant"].get(
        ...
        self.otel_collector_endpoint: str = self.config["otel"].get(
            "OTLP_HTTP_ENDPOINT", "http://localhost:4318"
        )

        self.vercel_ai_gateway_api_key: str = self.config["vercel"].get(
